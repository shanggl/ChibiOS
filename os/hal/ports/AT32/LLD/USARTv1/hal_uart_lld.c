...
static uartflags_t translate_errors(uint16_t sr) {
  ...
  if (sr & USART_ROERR_FLAG)
    sts |= UART_OVERRUN_ERROR;
  if (sr & USART_PERR_FLAG)
    sts |= UART_PARITY_ERROR;
  if (sr & USART_FERR_FLAG)
    sts |= UART_FRAMING_ERROR;
  if (sr & USART_NERR_FLAG)
    sts |= UART_NOISE_ERROR;
  if (sr & USART_BFF_FLAG)
...
static void usart_start(UARTDriver *uartp) {
  ...
  /* Note that some bits are enforced because required for correct driver
     operations.*/
  u->ctrl2 = uartp->config->cr2 | USART_CR2_LBDIE;
  u->ctrl3 = uartp->config->cr3 | USART_CR3_DMAT | USART_CR3_DMAR |
                                USART_CR3_EIE;
...
void uart_lld_init(void) {
  ...
  UARTD1.usart   = USART1;
  UARTD1.clock   = clocks_freq.apb2_freq;
  ...
  UARTD2.usart   = USART2;
  UARTD2.clock   = clocks_freq.apb1_freq;
  ...
  UARTD3.usart   = USART3;
  UARTD3.clock   = clocks_freq.apb1_freq;
  ...
  UARTD4.usart   = UART4;
  UARTD4.clock   = clocks_freq.apb1_freq;
  ...
  UARTD5.usart   = UART5;
  UARTD5.clock   = clocks_freq.apb1_freq;
  ...
  UARTD6.usart   = USART6;
  UARTD6.clock   = clocks_freq.apb2_freq;
...
#endif
    uartp->usart->sts = ~USART_SR_TC;
    uartp->usart->ctrl1 |= USART_CR1_TCIE;
  }

...
static void spi_lld_configure(SPIDriver *spip) {
  ...
  if (spip->config->slave) {
    spip->spi->ctrl1  = spip->config->cr1 & ~(SPI_CR1_MSTR | SPI_CR1_SPE);
    spip->spi->ctrl2  = spip->config->cr2 |
                      SPI_CR2_RXDMAEN | SPI_CR2_TXDMAEN;
  }
  else {
    spip->spi->ctrl1  = (spip->config->cr1 | SPI_CR1_MSTR) & ~SPI_CR1_SPE;
    spip->spi->ctrl2  = spip->config->cr2 | SPI_CR2_SSOE |
                      SPI_CR2_RXDMAEN | SPI_CR2_TXDMAEN;
  }
}
...
static msg_t spi_lld_stop_abort(SPIDriver *spip) {
  ...
  if (!spip->config->slave) {
    ...
    while ((spip->spi->sts & SPI_SR_BSY) != 0U) {
    }
    spip->spi->ctrl1 &= ~SPI_CR1_SPE;
...
uint16_t spi_lld_polled_exchange(SPIDriver *spip, uint16_t frame) {
  ...
  spip->spi->dt = frame;
  while ((spip->spi->sts & SPI_SR_RXNE) == 0U)
    ;
  frame = spip->spi->dt;

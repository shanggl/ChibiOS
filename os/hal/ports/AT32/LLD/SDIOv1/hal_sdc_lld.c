...
static bool sdc_lld_prepare_read_bytes(SDCDriver *sdcp,
  ...
  sdcp->sdio->dttmr = STM32_SDC_READ_TIMEOUT;
  ...
  sdcp->sdio->intclr   = STM32_SDIO_ICR_ALL_FLAGS;
  sdcp->sdio->inten  = SDIO_MASK_DCRCFAILIE |
  ...
  sdcp->sdio->dtctrl = SDIO_DCTRL_DTDIR |
...
static bool sdc_lld_wait_transaction_end(SDCDriver *sdcp, uint32_t n,
  ...
  osalSysLock();
  if (sdcp->sdio->inten != 0)
    osalThreadSuspendS(&sdcp->thread);
  if ((sdcp->sdio->sts & SDIO_STA_DATAEND) == 0) {
    osalSysUnlock();
    return HAL_FAILED;
  }
  ...
  while (sdcp->dma->stream->ctrl & STM32_DMA_CR_EN)
    ;
...
void sdc_lld_start(SDCDriver *sdcp) {
  ...
  if (sdcp->state == BLK_STOP) {
    ...
    dmaStreamSetPeripheral(sdcp->dma, &sdcp->sdio->buf);
      ...
      dmaSetRequestSource(sdcp->dma, STM32_DMAMUX1_SDIO1);
#endif
#if (defined(STM32F4XX) || defined(STM32F2XX))
...
void sdc_lld_start_clk(SDCDriver *sdcp) {
  ...
  uint32_t clk_psc = (system_core_clock / 200000) - 2;
  if(clk_psc > 0x3FF)
  {
    clk_psc = 0x3FF;
  }
  sdio_clock_config(sdcp->sdio, clk_psc, SDIO_CLOCK_EDGE_FALLING);
  sdcp->sdio->pwrctrl |= SDIO_POWER_PWRCTRL_0 | SDIO_POWER_PWRCTRL_1;
  sdcp->sdio->clkctrl |= SDIO_CLKCR_CLKEN;
...
void sdc_lld_set_data_clk(SDCDriver *sdcp, sdcbusclk_t clk) {
  ...
  if (SDC_CLK_50MHz == clk) {
	...
	sdcp->sdio->clkctrl_bit.clkdiv_l = (clkdiv & 0xFF);

	  /* config clock divide [9:8] */
	sdcp->sdio->clkctrl_bit.clkdiv_h = ((clkdiv & 0x300) >> 8);
#if 0
    sdcp->sdio->clkctrl = (sdcp->sdio->clkctrl & 0xFFFFFF00U) | STM32_SDIO_DIV_HS
                                                          | SDIO_CLKCR_BYPASS;
#endif
  }
  else
...
void sdc_lld_set_bus_mode(SDCDriver *sdcp, sdcbusmode_t mode) {
  uint32_t clk = sdcp->sdio->clkctrl & ~SDIO_CLKCR_WIDBUS;
  ...
  case SDC_MODE_1BIT:
    ...
    break;
  case SDC_MODE_4BIT:
    sdcp->sdio->clkctrl = clk | SDIO_CLKCR_WIDBUS_0;
    break;
  case SDC_MODE_8BIT:
    sdcp->sdio->clkctrl = clk | SDIO_CLKCR_WIDBUS_1;
    break;
  }
}
...
void sdc_lld_send_cmd_none(SDCDriver *sdcp, uint8_t cmd, uint32_t arg) {
  ...
  sdcp->sdio->argu = arg;
  sdcp->sdio->cmdctrl = (uint32_t)cmd | SDIO_CMD_CPSMEN;
  while ((sdcp->sdio->sts & SDIO_STA_CMDSENT) == 0)
    ;
  sdcp->sdio->intclr = SDIO_ICR_CMDSENTC;
}
...
bool sdc_lld_send_cmd_short(SDCDriver *sdcp, uint8_t cmd, uint32_t arg,
  ...
  sdcp->sdio->argu = arg;
  sdcp->sdio->cmdctrl = (uint32_t)cmd | SDIO_CMD_WAITRESP_0 | SDIO_CMD_CPSMEN;
  while (((sta = sdcp->sdio->sts) & (SDIO_STA_CMDREND | SDIO_STA_CTIMEOUT |
                                     SDIO_STA_CCRCFAIL)) == 0)
    ;
  sdcp->sdio->intclr = sta & (SDIO_STA_CMDREND | SDIO_STA_CTIMEOUT |
                           SDIO_STA_CCRCFAIL);
...
bool sdc_lld_send_cmd_short_crc(SDCDriver *sdcp, uint8_t cmd, uint32_t arg,
  ...
  while (((sta = sdcp->sdio->sts) & (SDIO_STA_CMDREND | SDIO_STA_CTIMEOUT |
                                     SDIO_STA_CCRCFAIL)) == 0)
    ;
  sdcp->sdio->intclr = sta & (SDIO_STA_CMDREND | SDIO_STA_CTIMEOUT | SDIO_STA_CCRCFAIL);
...
bool sdc_lld_send_cmd_long_crc(SDCDriver *sdcp, uint8_t cmd, uint32_t arg,
  ...
  sdcp->sdio->argu = arg;
  sdcp->sdio->cmdctrl = (uint32_t)cmd | SDIO_CMD_WAITRESP_0 | SDIO_CMD_WAITRESP_1 |
                                    SDIO_CMD_CPSMEN;
  while (((sta = sdcp->sdio->sts) & (SDIO_STA_CMDREND | SDIO_STA_CTIMEOUT |
                                     SDIO_STA_CCRCFAIL)) == 0)
    ;
...
bool sdc_lld_write_aligned(SDCDriver *sdcp, uint32_t startblk,
  ...
  sdcp->sdio->dttmr = STM32_SDC_WRITE_TIMEOUT;
  ...
  /* Transaction starts just after DTEN bit setting.*/
  sdcp->sdio->dtctrl = SDIO_DCTRL_DBLOCKSIZE_3 |
                      SDIO_DCTRL_DBLOCKSIZE_0 |
                      SDIO_DCTRL_DMAEN |
                      SDIO_DCTRL_DTEN;

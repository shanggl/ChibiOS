...
void adc_lld_start(ADCDriver *adcp) {
  ...
  if (adcp->state == ADC_STOP) {
    ...
    ADCCOM->cctrl = (ADCCOM->cctrl & (ADC_CCR_TSVREFE | ADC_CCR_VBATE)) |
               (STM32_ADC_ADCPRE << 16);
    ...
    adcp->adc->ctrl1 = 0;
    adcp->adc->ctrl2 = 0;
//    adcp->adc->ctrl2 = ADC_CR2_ADON;
    adc_enable(adcp->adc, TRUE);
    while(adc_flag_get(adcp->adc, ADC_RDY_FLAG) == RESET);
    /* adc calibration */
    adc_calibration_init(adcp->adc);
    while(adc_calibration_init_status_get(adcp->adc));
    adc_calibration_start(adcp->adc);
    while(adc_calibration_status_get(adcp->adc));
  }
}
...
void adc_lld_start_conversion(ADCDriver *adcp) {
  ...
  adcp->adc->vmhb   = grpp->htr;
  adcp->adc->vmlb   = grpp->ltr;
  adcp->adc->osq1  = grpp->sqr1 | ADC_SQR1_NUM_CH(grpp->num_channels);
  adcp->adc->osq2  = grpp->sqr2;
  adcp->adc->osq3  = grpp->sqr3;

  /* ADC configuration and start.*/
  adcp->adc->ctrl1 = grpp->cr1 | ADC_CR1_OVRIE | ADC_CR1_SCAN;
  ...
  if ((cr2 & ADC_CR2_SWSTART) != 0) {
    ...
    adcp->adc->ctrl2 = (cr2 | ADC_CR2_CONT) & ~ADC_CR2_SWSTART;

    /* Finally enabling ADC_CR2_SWSTART.*/
    adcp->adc->ctrl2 = (cr2 | ADC_CR2_CONT);
  }
  else
    adcp->adc->ctrl2 = cr2;
}
...
void adc_lld_stop_conversion(ADCDriver *adcp) {
  ...
  adcp->adc->ctrl2 &= ~(ADC_CR2_SWSTART);
  adcp->adc->ctrl2 = ADC_CR2_ADON;
}

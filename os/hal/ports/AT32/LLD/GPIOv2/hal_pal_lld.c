...
void _pal_lld_setgroupmode(ioportid_t port,
  ...
  while (true) {
    if ((mask & 1) != 0) {
      ...
      m2 = 3 << (bit * 2);
      m4 = 15 << ((bit & 7) * 4);
      port->OMODE  = (port->OMODE & ~m1) | otyper;
      port->ODRVR = (port->ODRVR & ~m2) | ospeedr;
      port->PULL   = (port->PULL & ~m2) | pupdr;
      if ((mode & PAL_STM32_MODE_MASK) == PAL_STM32_MODE_ALTERNATE) {
        /* If going in alternate mode then the alternate number is set
           before switching mode in order to avoid glitches.*/
        if (bit < 8)
          port->MUXL = (port->MUXL & ~m4) | altrmask;
        else
          port->MUXH = (port->MUXH & ~m4) | altrmask;
        port->CFGR   = (port->CFGR & ~m2) | moder;
      }
      else {
...
void _pal_lld_enablepadevent(ioportid_t port,
  ...
  // AT32 USING SCFG EXINT0 - 4 FOR PORT & PIN EXTI SOURCE SELECTION 
  uint32_t tmp = 0x00;
  tmp = ((uint32_t)0x0F) << (0x04 * (pad & (uint8_t)0x03));

  switch (pad >> 0x02)
  {
    case 0:
      SCFG->exintc1 &= ~tmp;
      SCFG->exintc1 |= (((uint32_t)portidx) << (0x04 * (pad & (uint8_t)0x03)));
      break;
    case 1:
      SCFG->exintc2 &= ~tmp;
      SCFG->exintc2 |= (((uint32_t)portidx) << (0x04 * (pad & (uint8_t)0x03)));
      break;
    case 2:
      SCFG->exintc3 &= ~tmp;
      SCFG->exintc3 |= (((uint32_t)portidx) << (0x04 * (pad & (uint8_t)0x03)));
      break;
    case 3:
      SCFG->exintc4 &= ~tmp;
      SCFG->exintc4 |= (((uint32_t)portidx) << (0x04 * (pad & (uint8_t)0x03)));
      break;
    default:
      break;
  }
...
#if STM32_EXTI_HAS_CR == FALSE
    // croff  = ((uint32_t)pad & 3U) * 4U;s
    // crport = (SYSCFG->EXTICR[cridx] >> croff) & 0xFU;
//TODO: add support
 //AT32 SCFG USING 4 32bit REG FOR PORT& PIN EXIT SOURCE SELECTION 
